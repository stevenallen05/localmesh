...
| Grafana auth | Static service-account token, env-injected. Dev: `make setup` resets admin/admin via `grafana-cli admin reset-admin-password`, then runs the profile-gated `grafana-bootstrap` one-shot which mints the token to a shared named volume. Explicit `make setup` (rather than init-container) so credential reset is visible and recoverable. | Production-shaped credential (Grafana 9+ replaces API keys with service accounts); single token, Bearer header. | Replace bootstrap with a Vault Agent / secrets-backend sidecar that writes the same file. Rust code unchanged. |
