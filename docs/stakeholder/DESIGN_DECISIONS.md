...
| Grafana auth | Static service-account token, env-injected. Dev: `make setup` runs `docker compose down -v` (sledgehammer reset — wipes all volumes), then mints a fresh token via the profile-gated `grafana-bootstrap` one-shot. Side-stepping persisted-state drift is cheaper than a granular reset path for a take-home. | Production-shaped credential (Grafana 9+ replaces API keys with service accounts); single token, Bearer header. | Replace `make setup`'s sledgehammer with a `--force`-gated granular reset; replace `grafana-bootstrap` with a Vault Agent / secrets-backend sidecar that writes the same file. Rust code unchanged. |
