...
def test_env_lines_strict_upcase_snakecase(tmp_path, monkeypatch):
    ...
    assert "LOCAL_DOMAIN=tw-demo.local" in text
...
def test_caddyfile_emits_route_for_exposed_services(tmp_path, monkeypatch):
    ...
    assert "www.demo.tw-demo.local:8443" in text
    assert "grafana.demo.tw-demo.local:8443" in text
    assert "caddy.demo.tw-demo.local:8443" not in text     # ingress=true: no self-route
